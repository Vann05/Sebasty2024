...
app = Flask(__name__)
CORS(app)
os.environ['GOOGLE_APPLICATION_CREDENTIALS'] = '/home/Sebasty/Desktop/tts/demo.json'
...
def admin():
    ...
    return render_template('Admin.html', surveys=surveys, average_ratings=average_ratings, overall_srating=overall_srating, unknown_questions=unique_unknown_questions, camera_feed_url=camera_feed_url)
...
# @app.route('/fqasynthesis', methods=['POST'])
# def fqasynthesis():
#     texts = request.json['texts']  # assuming the texts are sent as a JSON array
...
#         audio_files.append(response.audio_content)
